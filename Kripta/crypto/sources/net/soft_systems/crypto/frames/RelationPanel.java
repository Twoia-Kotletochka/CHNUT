...
public class RelationPanel extends JPanel {
	public RelationPanel(CryptoEditFrame cryptoEditFrame) {
		super();
		...
		if (RelationEnabledBean.class.isAssignableFrom(bean.getClass())) {
			...
			rightObjects = relationBean.getAccessedObjects();
		}
		else {
	...
	protected void init() {
		...
		allToRightButton.setText(">>");
		allToLeftButton.setText("<<");
		...
		rightScroll.setMinimumSize(new Dimension(100, 100));
		rightScroll.setPreferredSize(new Dimension(100, 150));
		...
		centerPanel.add(allToLeftButton,
			new GridBagConstraints(GridBagConstraints.RELATIVE, GridBagConstraints.RELATIVE,
			GridBagConstraints.REMAINDER, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL,
			new Insets(0, 5, 5, 5), 0, 0));
		centerPanel.add(allToRightButton,
			new GridBagConstraints(GridBagConstraints.RELATIVE, GridBagConstraints.RELATIVE,
			GridBagConstraints.REMAINDER, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL,
			new Insets(0, 5, 5, 5), 0, 0));
		// setting listeners
		...
		allToRightButton.addActionListener(
			new ActionListener() {
				public void actionPerformed(ActionEvent e) { moveAllToRight(); }
			});
		allToLeftButton.addActionListener(
			new ActionListener() {
				public void actionPerformed(ActionEvent e) { moveAllToLeft(); }
			});
	...
	protected void moveToRight() {
		Object[] selValues = leftList.getSelectedValues();
		for (int i = 0; i < selValues.length; i++) {
			Object obj = selValues[i];
	...
	protected void moveAllToRight() {
		rightObjects.addAll(leftObjects);
		leftObjects.clear();
		resetLists();
	}
	protected void moveToLeft() {
		Object[] selValues = rightList.getSelectedValues();
		for (int i = 0; i < selValues.length; i++) {
			Object obj = selValues[i];
			leftObjects.add(obj);
	...
	protected void resetLists() {
		if (relationBean != null) {
			...
			rightList.setListData(rightObjects);
		}
	}
	public void setRelations() {
		if (relationBean != null) { relationBean.setAccessedObjects(rightObjects); }
	}
	/**
	 * @associates Object
	 */
	...
	private JButton allToRightButton = new JButton();
	private JButton allToLeftButton  = new JButton();
