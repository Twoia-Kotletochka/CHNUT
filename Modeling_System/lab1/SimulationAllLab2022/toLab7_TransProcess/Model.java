...
public class Model implements IStatisticsable, IExperimentable, ITransProcesable {
	...
	public Model(Dispatcher d, TransGUI g) {
		if (d == null || g == null) {
	...
	public Map<String, ITransMonitoring> getMonitoringObjects() {
		double w =  getDevice().getRndDev().average()/
				getTransactGenerator().getRndGen().average();				;
		Map<String, ITransMonitoring> map = new HashMap<>();
		map.put(String.format("Черга заявок, w = %1.2f", w), (ITransMonitoring)getQueue());
		return map;
	}
